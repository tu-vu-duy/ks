...
public class UISplitTopicForm extends UIForumKeepStickPageIterator implements UIPopupComponent {
  ...
  protected List<Post> getListPost() throws Exception {
    ...
    if (isSetPage) {
      postListAccess = getForumService().getPostForSplitTopic(new PostFilter(path));
    }
  ...
  public void setPageListPost(PostListAccess listAccess) {
    this.postListAccess = listAccess;
    isSetPage = false;
  }
